...
const Button = styled('button', {
    ...
    ({ theme, open }) => css`
        ...
        background-color: ${theme.palette.mode === 'light' ? theme.palette.common.white : theme.palette.gray[1000]};
        border-radius: 8px;
...
const Items = styled('ol', {
    ...
    ({ theme, open }) => css`
        ...
        margin: 0.25rem 0 0;
        border: 1px solid ${theme.palette.mode === 'light' ? theme.palette.gray[200] : theme.palette.gray[900]};
        min-width: 110px;
        border-radius: 8px;
        list-style: none;
        padding: 0.75rem 0.75rem 0 0.75rem;
        background-color: ${theme.palette.mode === 'dark'
            ? MLAB_OPACITY_PALETTE.black[800]
            : MLAB_OPACITY_PALETTE.white[800]};
    `
);
            ...
            <ClickAwayListener onClickAway={() => setOpen(false)}>
                <Box position={'relative'}>
                    ...
                    <Items open={open}>
                        {items.map((item) => (
                            <Item
                                selected={getKey(item) === getKey(value)}
                                key={getKey(item)}
