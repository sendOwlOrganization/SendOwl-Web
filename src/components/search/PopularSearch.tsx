        ...
        <Grid item xs={6}>
            <Button onClick={onClick}>
                <Typography
                    ...
                    marginLeft={'0.25rem'}
                    fontWeight={'bold'}
                    color={(theme) => (index < 3 ? theme.palette.pink[600] : theme.palette.gray[800])}>
                    {index + 1}
                </Typography>
                {children}
            </Button>
    ...
    return (
        <SearchContainer title={'인기 검색어'}>
            <Grid container spacing={0.75}>
                {[0, 1, 2, 3, 4].map((i) => (
                    <Fragment key={i}>
                        <GridItem index={i} onClick={() => setSearch(words[i])}>
                            <Typography color={(theme) => theme.palette.common.black}>{words[i]}</Typography>
                        </GridItem>
                        <GridItem index={i + 5} onClick={() => setSearch(words[i + 5])}>
                            <Typography color={(theme) => theme.palette.common.black}>{words[i]}</Typography>
                        </GridItem>
                    </Fragment>
                ))}
